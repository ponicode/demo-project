...
function get_and_validate_department(args) {
  var req             = args.req,
      ...
      // If no_suffix is set then parameter names Preare considered without "indexes"
  ...
  if (!validator.isFloat(allowance)) {
    console.log('New allowance for '+department_name+' should be numeric')
  ...
  let j = 0
  for (j = 0; j < args.employees.length; j++){
    if (args.employees[j].employee_id == -1){
        console.log(args.employees[j].joined_date)
    }
  }
  return {
    allowance               : allowance,
